...
public class MinHeap<T extends Comparable<T>> implements PriorityQueue<T> {
	...
	void decrease(int loc) {
		...
		T currentValue = this.array[loc].getValue();
		int parentLoc = loc / 2;
		this.ticker.tick(2);
		
		while (parentLoc >= 1) {
			T parentValue = this.array[parentLoc].getValue();
			this.ticker.tick();
			
			if (currentValue.compareTo(parentValue) < 0) {
				// swap parent node with current node
				Decreaser<T> temp = this.array[parentLoc];
				this.array[parentLoc] = this.array[loc];
				this.array[loc] = temp;
	...
	private void heapify(int where) {
		...
		if (this.size < left) {
			...
			T currentValue = this.array[where].getValue();
			T leftValue = this.array[left].getValue();
			this.ticker.tick(2);
			
			if (this.size < right) {
				if (leftValue.compareTo(currentValue) < 0) {
					Decreaser<T> temp = this.array[left];
					this.array[left] = this.array[where];
					this.array[where] = temp;
					this.array[left].loc = left;
				...
				T rightValue = this.array[right].getValue();
				this.ticker.tick();
				if (leftValue.compareTo(rightValue) <= 0 
						&& leftValue.compareTo(currentValue) < 0) {
					Decreaser<T> temp = this.array[left];
					this.array[left] = this.array[where];
					this.array[where] = temp;
					this.array[left].loc = left;
					...
					this.ticker.tick(6);
						...
						&& rightValue.compareTo(currentValue) < 0) {
					Decreaser<T> temp = this.array[right];
					this.array[right] = this.array[where];
					this.array[where] = temp;
					this.array[right].loc = right;
					this.array[where].loc = where;
					next = where * 2 + 1;
					this.ticker.tick(6);
				}
