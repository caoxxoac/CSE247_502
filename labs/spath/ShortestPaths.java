...
public class ShortestPaths {
	...
	public void run() {
		...
		while (!pq.isEmpty()) {
			...
			Iterable<Edge> edgesFrom = vad.vertex.edgesFrom();
			
			for (Edge edge : edgesFrom) {
				Vertex vertexTo = edge.to;
				
				Decreaser<VertexAndDist> decreaser = this.handles.get(vertexTo);
				
				int newDistance = vad.distance + this.weights.get(edge);
				int distance = decreaser.getValue().distance;
				if (newDistance < distance) {
					this.parentEdges.put(vertexTo, edge);
					decreaser.decrease(new VertexAndDist(vertexTo, newDistance));
					this.handles.put(vertexTo, decreaser);
				}
			}
		}
	}
	...
	public LinkedList<Edge> returnPath(Vertex endVertex) {
		...
		while (!vertex.equals(this.startVertex)) {
			Edge edge = this.parentEdges.get(vertex);
			if (edge == null) {
				break;
			}
			path.addFirst(edge);
